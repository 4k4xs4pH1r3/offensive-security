    ...
    try:
        ...
        for reservation in response['Reservations']:
            for instance in reservation['Instances']:
                ...
                instance_ip = instance.get('PublicIpAddress')  # Get the public IP if available
                ...
                for sg in security_groups:
                        ...
                        for port_info in ports_to_open:
                            ...
                            if isinstance(port, list):
                                for p in range(port[0], port[1] + 1):
                                    thread = threading.Thread(target=test_port_reachability, args=(instance_ip, p, protocol, port_reachability_results))
                                    threads.append(thread)
                                    thread.start()
                            else:
                                thread = threading.Thread(target=test_port_reachability, args=(instance_ip, port, protocol, port_reachability_results))
                                threads.append(thread)
                                thread.start()
                    ...
                    for rule in existing_rules:
                        if rule['IpProtocol'] == 'tcp' or rule['IpProtocol'] == 'udp':
                            port_range = list(range(rule['FromPort'], rule['ToPort'] + 1)) if rule['FromPort'] != rule['ToPort'] else [rule['FromPort']]
                            for port in port_range:
                                if (port, rule['IpProtocol']) in port_reachability_results and port_reachability_results[(port, rule['IpProtocol'])]:
                                    if 'CidrIpv4' in rule and rule['CidrIpv4'] == '0.0.0.0/0':
                                        rules_to_remove.append(rule['SecurityGroupRuleId'])
                    ...
                    if rules_to_remove:
                        try:
                            ...
                            for rule_id in rules_to_remove:
                                print(f"    Removed rule allowing traffic from 0.0.0.0/0 on {rule['IpProtocol']} port in security group {sg_name} ({sg_id})")
                        except Exception as e:
                    ...
                    for port_info in ports_to_open:
                        ...
                        for cidr in allowed_cidrs:
                            ...
                            if isinstance(port, list):  # Handle port ranges
                                rule_exists = any(
                                    rule['IpProtocol'] == protocol and
                                    rule['FromPort'] <= port[0] and
                                    rule['ToPort'] >= port[1] and
                                    rule['CidrIpv4'] == cidr
                                    for rule in existing_rules
                                )
                                ...
                                rule_exists = any(
                                    rule['IpProtocol'] == protocol and
                                    rule['FromPort'] == port and
                                    rule['ToPort'] == port and
                                    rule['CidrIpv4'] == cidr
                                    for rule in existing_rules
                                )
                    ...
                    if rules_to_add:
                        try:
                            ...
                            for rule in rules_to_add:
                                if isinstance(rule['FromPort'], int) and rule['FromPort'] == rule['ToPort']:
                                    print(f"    Added rule: {rule['IpProtocol']}/{rule['FromPort']} from {rule['IpRanges'][0]['CidrIp']} to {sg_name} ({sg_id})")
                                else:
                                    print(f"    Added rule: {rule['IpProtocol']}/{rule['FromPort']}-{rule['ToPort']} from {rule['IpRanges'][0]['CidrIp']} to {sg_name} ({sg_id})")

                        except ec2.exceptions.ClientError as e:
                            if e.response['Error']['Code'] == 'RulesPerSecurityGroupLimitExceeded':
                                print(f"    Error: RulesPerSecurityGroupLimitExceeded for {sg_name} ({sg_id}).")

                                # Consolidate rules for the specified ports
                                consolidated_rules = consolidate_rules(rules_to_add, ports_to_open)
                                ...
                                try:
                                    ...
                                    for rule in consolidated_rules:
                                        if isinstance(rule['FromPort'], int) and rule['FromPort'] == rule['ToPort']:
                                            print(f"    Added consolidated rule: {rule['IpProtocol']}/{rule['FromPort']} to {sg_name} ({sg_id})")
                                        else:
                                            print(f"    Added consolidated rule: {rule['IpProtocol']}/{rule['FromPort']}-{rule['ToPort']} to {sg_name} ({sg_id})")

                                except ec2.exceptions.ClientError as e:
                                    if e.response['Error']['Code'] == 'RulesPerSecurityGroupLimitExceeded':
                                        print(f"    Error: RulesPerSecurityGroupLimitExceeded even after consolidation for {sg_name} ({sg_id}).")
                                        # Create a new security group
                                        new_sg_id = create_new_security_group(ec2, sg_name, instance['VpcId'], consolidated_rules, allowed_cidrs, sg_id)

                                        if new_sg_id:
                                            print(f"    Created new security group: {new_sg_id}")

                                            # Attach the new security group to the instance
                                            current_group_ids = [sg['GroupId'] for sg in security_groups]
                                            if new_sg_id not in current_group_ids:
                                                ec2.modify_instance_attribute(InstanceId=instance_id, Groups=(current_group_ids + [new_sg_id]), DryRun=dry_run)
                                                print(f"    Attached new security group {new_sg_id} to instance {instance_id}")
                                            else:
                                                print(f"    Security group {new_sg_id} is already attached to instance {instance_id}")
...
def consolidate_rules(rules, ports_to_open):
    ...
    for port_info in ports_to_open:
        ...
        for rule in rules:
            if isinstance(port, list):  # Handle port ranges
                if rule['FromPort'] == port[0] and rule['ToPort'] == port[1] and rule['IpProtocol'] == protocol:
...
def create_new_security_group(ec2, original_sg_name, vpc_id, rules, allowed_cidrs, original_sg_id):
    ...
    existing_sgs = ec2.describe_security_groups(Filters=[{'Name': 'vpc-id', 'Values': [vpc_id]}])['SecurityGroups']
    count = 1
    ...
    try:
        ...
        print(f"    Added consolidated rules to the new security group: {new_sg_id} ({new_sg_name})")

        return new_sg_id

    except ec2.exceptions.ClientError as e:
        if e.response['Error']['Code'] == 'InvalidGroup.Duplicate':
            print(f"    Error creating new security group: Security group '{new_sg_name}' already exists.")
            ...
            if sg_response['SecurityGroups']:
                ...
                print(f"    Existing security group with name '{new_sg_name}' found: {existing_sg['GroupId']}")
