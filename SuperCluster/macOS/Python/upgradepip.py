    ...
    try:
        ...
        if installation_method:
            ...
            if "Version" in installed_result.stdout:
                ...
                if (
                    installation_method.startswith("pip")
                    and "requirements.txt" not in sys.argv
                ):
                    print(
                        f"{Fore.YELLOW}Attempting to install {package} using brew...{Style.RESET_ALL}"
                    )
                    ...
                    if "Error: No available formula" in brew_install_result.stderr:
                        print(
                            f"{Fore.RED}Failed to install {package} using brew: Package not found in Homebrew.{Style.RESET_ALL}"
                        )
                    else:
                        print(
                            f"{Fore.GREEN}Successfully installed {package} using brew{Style.RESET_ALL}"
                        )
        else:
            print(
...
def install_prerequisites():
    try:
        ...
        subprocess.run(
            ["python3", "-m", "pip", "install", "types-colorama"], check=True
        )
